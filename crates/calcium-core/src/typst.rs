...
//!
//! A quantity — a number against nothing but units — becomes one `#unit[...]`
//! call, exponents and all: `31.6144 pW/sqrt(Hz)` typesets as
//! `31.6144 #unit[pW Hz^-0.5]`. Fractional exponents print as decimals
//! because fancy-units has no radical form and rejects `^(1/2)`.
...
fn assemble(pieces: Vec<Piece>) -> String {
    let mut out: Vec<String> = vec![
        "// Generated by `calcium typst` — edit the .calcium source instead.\n\
         #import \"@preview/fancy-units:0.1.1\": fancy-units-configure, unit\n\
...
fn statement_holds_error(statement: &Statement) -> bool {
    fn walk(expr: &Expr) -> bool {
        ...
        let mut found = false;
        let mut check = |e: &Expr| found |= walk(e);
        match expr {
    ...
    match &statement.stmt {
        Stmt::Define { body, .. } => walk(body),
        Stmt::Equation { lhs, rhs } => walk(lhs) || walk(rhs),
        Stmt::Expr(expr) => walk(expr),
        Stmt::SumDefine { .. } => false,
        Stmt::Directive { value, .. } => value.as_ref().map(walk).unwrap_or(false),
    }
}
...
impl Renderer {
            ...
            match block.kind {
                ...
                BlockKind::Code => {
                    ...
                    for statement in parse_line(&joined) {
                        if let Stmt::Define { name, .. } | Stmt::SumDefine { name } =
                            &statement.stmt
                        {
    ...
    /// Whether a name reads as a unit here: the prelude says so, the
    /// document has not shadowed it — a document that defines `T` means its
    /// T, not the tesla — and no symbol was declared for it.
    fn is_unit(&self, name: &str) -> bool {
        !self.defined.contains(name)
            && !self.symbols.contains_key(name)
            && self.env.is_unit_name(name)
    }
    ...
    /// typesets as the number and a single `#unit[...]` call.
    fn try_quantity(&self, factors: &[Expr]) -> Option<String> {
        ...
        let tight = atoms.len() == 1 && atoms[0].1.is_one() && tight_unit(&unit);
        let sep = if tight { "" } else { " thin " };
        Some(format!("{}{sep}{unit}", self.bare(coefficient)))
    }

    /// One `#unit[...]` call for a set of unit atoms. Division is written
    /// with a slash so the configured `per-mode` decides its face, and
    /// exponents are decimal because fancy-units panics on fraction forms
    /// like `^(1/2)`.
...
mod tests {
    ...
    fn symbols_section_renames_and_typesets_as_a_table() {
        ...
        assert!(
            out.contains("I &= I_\"th\" + 3 thin #unit[mA] = 53"),
            "got:\n{out}"
        );
    ...
    fn units_use_fancy_units_and_unmapped_names_become_text() {
        ...
        assert!(
            out.contains("$ \"laser power\" = 2 thin #unit[mW] $"),
            "got:\n{out}"
        );
        assert!(
            out.contains("#import \"@preview/fancy-units:0.1.1\": fancy-units-configure, unit"),
            "got:\n{out}"
        );
    ...
    fn prefixed_units_are_recognized() {
        ...
        assert!(out.contains("31.6 thin #unit[pW]"), "got:\n{out}");
    }

    #[test]
    fn per_root_hertz_becomes_a_slash_with_a_decimal_exponent() {
        let out = to_typst("    a = 31.6144 pW/sqrt(Hz)\n    b = 1 nA/sqrt(Hz)\n    c = 1/sqrt(Hz)\n");
        assert!(out.contains("31.6144 thin #unit[pW / Hz^0.5]"), "got:\n{out}");
        assert!(out.contains("1 thin #unit[nA / Hz^0.5]"), "got:\n{out}");
    ...
    fn scientific_notation_typesets_as_a_power_of_ten() {
        ...
        assert!(out.contains("2.5 times 10^(-19) thin #unit[J]"), "got:\n{out}");
    }
    ...
    fn conversions_vanish_from_the_typeset_form() {
        ...
        assert!(
            out.contains("\"speed\" = 60 thin #unit[mph] = 26.8224 thin #unit[m / s]"),
            "got:\n{out}"
        );
    }
    ...
    fn unindented_definitions_still_typeset_as_equations() {
        ...
        assert!(out.contains("T &= 125 thin #unit[degC]"), "got:\n{out}");
        assert!(
